...
const doc = `
Usage:
  cyware [options] list-upgrades [--pre-release]
  cyware [options] install [--pre-release] [--version=<version>] [--mode=<mode>] [--user=<user>]
  cyware [options] update
  cyware [options] upgrade [--pre-release] [--mode=<mode>] [--user=<user>]
  cyware [options] version
  cyware [options] debug
  cyware -h | --help | -v

Options:
  --dev                 Developer Mode (do not use, dangerous, bypasses checks)
  --version=<version>   Specific version install [default: latest]
  --mode=<mode>         CYWare installation mode (dedicated, addon, or cloud)
  --user=<user>         User used for CYWare configuration [default: ${currentUser}]
...
saltstack.log file under /var/cache/cyware/cli in the
subdirectory that matches the CYWare version you're installing.
Search for the log file for "result: false" messages and look at
the surrounding lines to diagnose the issue.

For assistance go to https://github.com/CYWare/cyware-cli/issues

`

let osVersion = null
let osCodename = null
let cachePath = '/var/cache/cyware/cli'
let versionFile = '/etc/cyware-version'
let configFile = '/etc/cyware-config'
let releaseFile = '/etc/os-release'
let cywareConfiguration = {}
...
const setup = async () => {
  if (cli['--dev'] === true) {
    cachePath = '/tmp/var/cache/cyware'
    versionFile = '/tmp/cyware-version'
    configFile = '/tmp/cyware-config'
  }
...
const downloadReleaseFile = (version, filename) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = request.get(`https://github.com/CYWare/salt-states/releases/download/${version}/${filename}`)
    req.on('error', (err) => {
      reject(err)
    })
    req
...
const downloadRelease = (version) => {
  console.log(`>> downloading cyware-salt-states-${version}.tar.gz`)

  const filepath = `${cachePath}/${version}/cyware-salt-states-${version}.tar.gz`
  ...
  return new Promise((resolve, reject) => {
    const output = fs.createWriteStream(filepath)
    const req = request.get(`https://github.com/CYWare/salt-states/archive/${version}.tar.gz`)
    req.on('error', (err) => {
      reject(err)
    })
    req
...
const downloadUpdate = async (version) => {
  ...
  await downloadReleaseFile(version, `cyware-salt-states-${version}.tar.gz.asc`)
  await downloadReleaseFile(version, `cyware-salt-states-${version}.tar.gz.sha256`)
  await downloadReleaseFile(version, `cyware-salt-states-${version}.tar.gz.sha256.asc`)
  await downloadRelease(version)
  await validateFile(version, `cyware-salt-states-${version}.tar.gz`)
  await validateSignature(version, `cyware-salt-states-${version}.tar.gz.sha256`)
  await extractUpdate(version, `cyware-salt-states-${version}.tar.gz`)
}
...
const performUpdate = (version) => {
  ...
  const stateApplyMap = {
    'dedicated': 'cyware.dedicated',
    'addon': 'cyware.addon',
    'cloud': 'cyware.cloud'
  }
 
  if (!isModeSpecified) {
    let savedMode = cywareConfiguration['mode']
    if (validModes.indexOf(savedMode) != -1) {
      cli['--mode'] = savedMode
	    console.log(`> using previous mode: ${cli['--mode']}`)
    }  else {
      console.log(`> no previous CYWare version found; performing a new 'dedicated' installation.`)
      cli['--mode'] = "dedicated"
    }
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    const updateArgs = [
      ...
      `pillar={cyware_user: "${cywareConfiguration['user']}"}`
    ]
...
const run = async () => {
  ...
  console.log(`> cyware-cli@${cfg.version}`)
...
`
  ...
  cywareConfiguration = await loadConfiguration()

  const version = await getCurrentVersion()
  console.log(`> cyware-version: ${version}\n`)
  ...
  if (cli['update'] === true) {
    if (version === 'notinstalled') {
      throw new Error('CYWare is not installed, unable to update.')
    }
  ...
  if (cli['install'] === true) {
    ...
    if (currentVersion !== 'notinstalled') {
      console.log('CYWare is already installed, please use the \"update\" or \"upgrade\" command.')
      return process.exit(0)
    }
    ...
    if (cli['--version'] === 'latest') {
      ...
      if (validRelease === false) {
        console.log(`${cli['--version']} is not a CYWare valid release.`)
        return process.exit(5)
      }
